...
export default {
  ...
  methods: {
    ...
    getchuju() {
      ...
      getAction('/dhjcmb/fpdkjcYxzl/queryByPid', obj)
        .then(res => {
          if (res.success == true) {
            console.log(res)
            ...
            for (let j = 0; j < xcjc.length; j++) {
              ...
              shuzu[0].fileList.push({ name: '', thumbUrl: image, uid: xcjc[j].id, zhuang: false })
            }
    ...
    shanchua(e) {
      ...
      deleteAction('/dhjcmb/fpdkjcYxzl/delete', obj).then(res => {
        if (res.success == true) {
    ...
    unpimg(dizhi, a, s) {
      ...
      postAction('/dhjcmb/fpdkjcYxzl/add', obj).then(res => {
        if (res.success == true) {
          console.log(res)
