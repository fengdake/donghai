<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator" style="margin-bottom:20px;">
      ...
      <!-- <a-button type="primary" icon="download" @click="shenhe('合规-员工异常行为-汇总表')">审核</a-button> -->
    ...
    <!-- table区域-end -->
    <!-- 表单区域 -->
    <duogeguiyuan ref="modalForm" :chenshua="chenshua" :shenhea="shenhea" :shencha="shencha"  @shuaxin="shuaxin"></duogeguiyuan>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/business/hGYcxwHzb/cklist',
  ...
  methods: {
    ...
    chenshu(){
      ...
      this.shencha = false
      if( list.zt == 0 ){
        this.$refs.modalForm.title = '陈述'
        this.$refs.modalForm.dakai(list)
      }else{
        this.$notification.warn({
          message: '提示',
          description: '请选择选择状态为未陈述得数据',
          duration: 3
        })
      }
    },
