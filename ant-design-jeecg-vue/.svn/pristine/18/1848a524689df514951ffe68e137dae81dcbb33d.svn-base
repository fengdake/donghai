...
export default {
  ...
  data() {
    return {
      ...
      cha:false,
      url: {
        list: '/business/dhglFxfl/zhspList',
    ...
    shenpia(){
      if (this.selectedRowKeys.length == 0) { 
        this.$notification.warn({
          message: '提示',
          description: `请选择信贷部审批的数据`,
          duration: 3
        })
      } else {
        for (let i = 0; i < this.dataSource.length; i++) {
          if (this.selectedRowKeys[0] == this.dataSource[i].id) {
            if( this.dataSource[i].spwczt == "信贷部审批" ){
              this.$refs.shenpitan.dakai(this.dataSource[i])
            }else{
              this.$notification.warn({
                message: '提示',
                description: `请选择信贷部审批的数据`,
      ...
      getAction('/business/dhglFxfl/queryByZjhm', obj)
        .then(res => {
          if( e=="dg" ){
            ...
            this.$refs.modalForma.dakai(res)
          }else if( e == "xe" ){
            console.log(res)
