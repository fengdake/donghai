...
export default {
  ...
  methods: {
    ...
    getchuju() {
      ...
      this.spinning = true
      getAction('/dhjcmb/fpdkjc/queryById', obj)
        .then(res => {
          console.log(res)
          if (res.success == true) {
            ...
            this.jibeninfo[4].value = res.result.fdrq //放贷日期
            this.jibeninfo[5].value = res.result.dqrq //到期日期
            this.jibeninfo[6].value = res.result.dbfs //担保方式
            this.jibeninfo[7].value = res.result.dkyt //贷款用途
            this.jibeninfo[8].value = res.result.zffs //支付方式
          } else {
            // 接口失败
          }
        })
    ...
    baocun() {
      ...
      let obj = {
        ...
        fdrq: this.jibeninfo[4].value, //起始日期
        dqrq: this.jibeninfo[5].value, //起始日期
        dbfs: this.jibeninfo[6].value, //担保方式
        dkyt: this.jibeninfo[7].value, //贷款用途
        zffs: this.jibeninfo[8].value //支付方式
      }
      this.spinning = true
      putAction('/dhjcmb/fpdkjc/edit', obj)
        .then(res => {
          console.log(res)
          if (res.success == true) {
