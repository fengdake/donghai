      ...
      <a-form :form="form">
        ...
        <a-form-item label="职位" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select
            style="width: 100%"
            placeholder="请选择职位"
            v-model="selectPosition" @change="changePosition()">
          <a-select-option v-for="(item,index) in selPositionVal" :key="index.toString()" :value="item.value">
            {{ item.text }}
          </a-select-option>
          </a-select>
        ...
        <template v-show="!roleDisabled">
          <a-form-item label="岗位分配" :labelCol="labelCol" :wrapperCol="wrapperCol" >
            <a-select mode="tags" v-model="selectedRole" size="default" placeholder="请选择用户岗位"  :filterOption="filterOption">
              <a-select-option
                v-for="(item,index) in roleList"
                :key="index"
                :value="item.id"
              >{{item.roleCode}}  {{item.roleName}}</a-select-option>
            </a-select>
          </a-form-item>
        </template>
        ...
        <a-form-item label="虚拟行员标记" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select v-decorator="[ 'xnhybj', {}]" placeholder="请选择虚拟行员标记">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          ...
          sysJobList:"/system/sysJob/getSysJobs",//查询岗位
          sysJueseList:"/sys/role/queryByPid",//查询岗位
        },
      }
    },
    created () {
    ...
    watch:{
      checkedDepartNameString:function() {
        if(this.checkedDepartNameString == '' || this.selectedDepartKeys.length < 1)
        {
            return;
        }
        httpAction(this.url.sysPositionList, { type: this.selectedDepartKeys[0] }, 'post').then((res) => {
          if (res.success) {
            this.selPositionVal = res.result;
          } else {
          }
        })
      }
    ...
    methods: {
      ...
      initialRoleList(record){

        if (record.userPosition != null && record.userPosition != '')
        {
          getAction(this.url.sysJueseList, { pid: record.userPosition }, 'get').then((res) => {
            if (res.success) {
        ...
        getAction(this.url.sysJueseList, { pid: '-1' }, 'get').then((res) => {
          if (res.success) {
            this.roleList = res.result;
          } else {
          }
      ...
      edit (record) {
        ...
        that.$nextTick(() => {
          that.form.setFieldsValue(pick(this.model,'username','sex','realname','email','phone','activitiSync','zjhm','xnhybj'))
        });
        // 调用查询用户对应的部门信息的方法
        ...
        if (record.userPosition != null && record.userPosition != '')
        {
          this.selectPosition = record.userPosition;
        }
      ...
      locadPostAndJob()
      {
        httpAction(this.url.sysPositionList, { type: this.checkedDepartKeys[0] }, 'post').then((res) => {
          if (res.success) {
            this.selPositionVal = res.result;
          } else {
          }
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            formData.userPosition = this.selectPosition.length > 1 ? this.selectPosition : '';
            formData.userJob = '';
      ...
      modalFormOk (formData) {
        ...
        if (tempS != this.checkedDepartNameString)
        {
          this.selectPosition = [];
          this.selPositionVal = [];
          this.selectJob = [];
