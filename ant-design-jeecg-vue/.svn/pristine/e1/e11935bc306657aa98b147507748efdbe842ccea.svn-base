        ...
        <div style="height:257px">
          ...
          <div class="jinduc" v-else>
            ...
            <p class="jinduc_a">审批结果:
              ...
              <span v-if="item.approvaltype.indexOf('申请')!=-1 ">{{item.approvaltype}}</span>
            </p>
            <p class="jinduc_a">审批意见:{{item.comment == "null" ? '暂无':item.comment}}</p>

            <img v-show="item.list.length > 0 && item.list[0].url != ''" v-for="( itema,indexa ) in item.list " :key="indexa" :src="mgerver+'/'+itema.url" style="width: 172px; height: 115px;" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    console.log(e)
      ...
      getAction('/fkjmxkhsq/fkJmxkhsqSqxx/getProcessHistortAndAttachment', obj).then(res => {
        //console.log(res)
        if (res.success == true) {
          //console.log('接口成功')
