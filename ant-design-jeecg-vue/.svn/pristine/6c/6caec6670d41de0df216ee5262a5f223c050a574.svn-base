<template>
  <div >
     <p></p>
    <a-carousel :dots="dots" ref="varousel" dotsClass="slick-dots slick-thumb" style="height:70%;background:#fff;">
      <!-- <a slot="customPaging" slot-scope="props">
        <img :src="getImgUrl(props.i)" />
      </a> -->

      <div v-for="(item,index) in list" :key="index" style="width:100%;height:80%;">
        <div style="height:400px;position: relative;overflow: hidden;display: flex;justify-content: center;align-items: center;">
          <!-- {{lista[index].width}} -->
          <img
        ...
        <div class="xiabian">
          <p class="fangsa">
						...
						<span>
							...
							<img v-if="cha==true&&title!=='影像资料总览'"   @click="shanchu(index)" src="../../../assets/shanchu.png"/> 
						</span>
						<span>
								<img v-if="index != 0" @click="shangyi(index)" src="../../../assets/shangyi.png"/> 
								<img  v-if="index!= list.length-1" @click="xiayi(index)" src="../../../assets/xiayi.png"/> 
						</span>
          </p>
        </div>
      </div>
    </a-carousel>
		<div class="xiabiana"> <img v-for="(item,index) in list" :class=" ci==index?'buzhe':'zhezao' " @click="dianji(index)" :key="index"  :src="item" /> </div>
  </div>
</template>
<script>
