        ...
        <a-row :gutter="24">
          ...
          <!-- <a-col :md="6" :sm="8">
              <a-form-item label="客户类型">
                <a-select placeholder="请选择" v-model="queryParam.khlx">
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/business/dhglJjXcjydx/xdsplist'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    kaishiShenpi() {
      if (this.selectedRowKeys.length == 0) {
        ...
        for (let i = 0; i < this.dataSource.length; i++) {
          for (let j = 0; j < this.selectedRowKeys.length; j++) {
            if (this.selectedRowKeys[j] == this.dataSource[i].id) {
              zjhmList.push(this.dataSource[i].zjhm)
            }
          }
        }
        var zjhm = `${zjhmList}`
        console.log(zjhm)
        console.log(`${this.selectedRowKeys}`)
        this.$refs.shenpiForm.dakai(zjhm, `${this.selectedRowKeys}`)
      }
    },
    //查看采集
    chakancaiji() {
