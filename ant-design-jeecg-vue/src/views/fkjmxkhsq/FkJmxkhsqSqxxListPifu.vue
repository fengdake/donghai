<template>
  <a-card :bordered="false">
    ...
    <fkjmxkhsq ref="child" @diaoyong="diaoyong" :sfzh="sfzh" :dangqian="dangqian"></fkjmxkhsq>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/fkjmxkhsq/fkJmxkhsqSqxx/listPifu'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    approval() {
      if (this.selectedRowKeys.length == 0) {
        ...
        if (this.selectionRows[0].status != '待审批') {
          this.$notification.warn({
            message: '提示',
          ...
          this.$refs.child.dakai(
            ...
            false, //是否显示按钮，false为不显示
            true //批复特有，根据判断批复信息页面是否显示按钮
          )
        }
      }
    },
