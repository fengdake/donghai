      ...
      <div :style="persryle">
        ...
        <div class="zhuti">
          ...
          <div class="zhuti_right">
            <!-- 整改通知书 -->
            <zgtzs v-show="name=='整改通知书'" ref="tzs" :id="id" :see="see"></zgtzs>
          </div>
        </div>
      </div>
    </template>
...
import zgtzs from './zgtzs' //现场下发整改通知
...
export default {
  ...
  data() {
    return {
      ...
      name: '系统数据',
      xuhao: 0,
      biglist: [{ name: '整改通知书', status: true }],
      see: false, //是否为查看
      id: ''
    }
  },
  ...
  methods: {
    ...
    _biglist(e) {
      ...
      console.log(e)
      if (this.biglist[e].name == '整改通知书') {
        this.$refs.tzs.getchuju()
      }
      this.name = this.biglist[e].name
      this.modal.title = '整改情况录入-' + this.biglist[e].name
    },
    ...
    dakai(a, b) {
      ...
      this.id = b
      this.biglist = [{ name: '整改通知书', status: true }]
      that.name = '整改通知书'
      that.modal.title = '整改情况录入-' + that.name
      that.modal.visible = true
        setTimeout(function() {
          that.$refs.tzs.getchuju()
        }, 20)
    },
