<template>
 <a-modal
  ...
  <div class="zhengxinaa">
    <!-- 征信头部 -->
    <div class="jia_topsw">
      <!-- <a-button type="primary" icon = 'save' v-show="cha && grantId == ''" @click="baocunxinxi">保存</a-button>
      <a-button
    ...
    <div style="display:flex;margin:20px auto;justify-content: space-between;" :style="'min-height:'+gaodu+'px'">
      <section
        ref="print"
        id="shouQuanShua"
        class="abcdefg"
      ...
      <div
        ...
        v-show=" index <1 "
      >
        <!-- <p style="text-align: right;padding:15px;    display: flex;justify-content: flex-end;" v-show=" grantId == ''">
          <a-button type="primary" @click="shanchua()" v-show="imageadressa!==''"><a-icon type="minus"  />删除</a-button>
           ...
           <input style="cursor:pointer;
                  ...
                  width: 100%;
                  border: none;
                  opacity: 0;top:0px;left:0px;position: absolute;" type="file" name="file" accept="image/*"  v-show="grantId==''" @change="tirggerFile($event)" />
          </button>
          </p> -->
          <viewer :images="imgs">
            <img :src="imageadressa" style="padding: 15px 35px;width: 100%;display: block;" @click="yulanaa(imageadressa)" v-show="imageadressa!==''" />
          </viewer>
      ...
      >
        <!-- <p style="text-align: right;padding:15px;    display: flex;justify-content: flex-end;" v-show=" grantId == ''">
          <a-button type="primary" @click="shanchub()" v-show="imageadressb!==''"><a-icon type="minus"  />删除</a-button>
           <button type="primary" style="margin-left:10px;position: relative;width:70px;height:32px;padding: 0px;border-radius: 5px;color: #fff;background-color: #1890ff;border-color: #1890ff;text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.12); box-shadow: 0 2px 0 rgba(0, 0, 0, 0.045);">
            上传
           <input style="cursor:pointer;
                  height: 100%;
                  width: 100%;
                  border: none;
                  opacity: 0;top:0px;left:0px;position: absolute;" type="file" name="file" accept="image/*"  v-show="grantId==''" @change="tirggerFilea($event)" />
          </button>
          </p> -->
          <viewer :images="imgs">
            <img :src="imageadressb" style="padding: 15px 35px;width: 100%;display: block;"  @click="yulanaa(imageadressb)" v-show="imageadressb!==''" />
          </viewer>
          
          <div style="padding:15px;"  v-show="imageadressb==''" >
          <h3 style="font-size:12px;color:#535353;text-align: center;margin-bottom:10px;">申请人配偶查询授权书</h3>
          <p
            style="font-size:12px;color:#535353;text-align: left;text-indent: 30px;"
    ...
    <div style="display:none">
      ...
      <div
        ...
        v-show=" index <1 "
      >
        <img :src="imageadressa" style="padding: 15px 35px;width: 100%;display: block;" v-show="imageadressa!==''" />
      ...
      <div
        ...
        v-show=" index >0"
      >
        <img :src="imageadressb" style="padding: 15px 35px;width: 100%;display: block;" v-show="imageadressb!==''" />
          <div style="padding:15px;"  v-show="imageadressb==''" >
          <h3 style="font-size:12px;color:#535353;text-align: center;margin-bottom:10px;">申请人配偶查询授权书</h3>
          <p
            style="font-size:12px;color:#535353;text-align: left;text-indent: 30px;line-height:1.8;"
  ...
  methods: {
   ...
   baocunxinxi() {
    ...
    getauthJtxx() {
      ...
      getAction('business/sxsqDbrxx/queryJtxx', obj).then(res => {
        //console.log(res)
         if (res.code == 500) {
