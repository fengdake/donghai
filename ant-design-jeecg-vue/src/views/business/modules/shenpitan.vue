<template>
  ...
  >
    <div style="padding: 24px;">
      ...
      <div class="jieguo">
        <span class="jieguoa" style="line-height: 40px;">审批状态:</span>
        <div>
          <a-radio-group @change="onChangea" v-model="valuea">
            <a-radio :style="radioStyle" value="bh">驳回</a-radio>
            <a-radio :style="radioStyle" value="th" v-if="zhihang==false">退回</a-radio>
            <a-radio :style="radioStyle" value="tg">通过</a-radio>
          </a-radio-group>
        </div>
      </div>

      <div class="jieguo">
        <span class="jieguoa" style="line-height: 40px;">环节:</span>
        ...
        <a-select
          ...
          @change="reyuanchange"
        >
          <a-select-option key="支行审批">支行审批</a-select-option>
        </a-select>
        <div v-else>{{shuju.spwczt}}</div>
      </div>
      <div class="jieguo" style="margin-top:15px;">
        <span class="jieguoa" style="line-height: 40px;">审批意见:</span>
        <div>
          <textarea
            style="width: 210px;height: 100px;font-size:12px;"
            v-model="spyj"
            placeholder="请输入审批意见"
          ></textarea>
        </div>
  ...
  methods: {
    ...
    baocun() {
      ...
      if (this.valuea == 'th') {
        ...
        this.huanjie = this.shuju.spwczt
			}
			// console.log(  )
      ...
      let obj = {
        ...
        spbs: this.valuea,
        spyj: this.spyj,
        fxflId: this.shuju.id
      }
      putAction('/business/dhglFxfl/activeSp', obj).then(res => {
        console.log(res)
        if (res.success) {
