<template>
  <a-card :bordered="false">
    ...
    <!-- s首贷查看 -->
    <scdh ref="shoudaia" @diaoyong="diaoyong" @ok="modalFormOk"></scdh>

    <fpdk ref="shoudaib" @diaoyong="diaoyong" @ok="modalFormOk"></fpdk>
    <!-- 个人贷款 -->
    <jctc ref="gerenjing" @ok="modalFormOk"></jctc>
    <!-- 对公贷款 -->
    <dgdk ref="duigong" @diaoyong="diaoyong" @ok="modalFormOk"></dgdk>
    <!-- 表单区域 -->
  </a-card>
</template>
...
import fpdk from './modules/fpdk'
import jctc from './modules/jctc' // 个人经营性贷款
import dgdk from './modules/dgdkModal' //对公贷款
export default {
  name: 'DhSpList',
  mixins: [JeecgListMixin],
  components: {shenpitana,scdh,jctc,dgdk,fpdk},
  data() {
    return {
        ...
        for (let i = 0; i < this.dataSource.length; i++) {
          if (this.selectedRowKeys[0] == this.dataSource[i].id) {
              if( this.dataSource[i].jclx == "首贷" ){
                this.$refs.shoudaia.dakai(this.dataSource[i],'1')
              }else if( this.dataSource[i].jclx == "扶贫贷款" ){
                this.$refs.shoudaib.dakai(this.dataSource[i],'1')
              }
              else if( this.dataSource[i].jclx == "个人贷款" ){
                console.log( "个人贷款" )
                this.$refs.gerenjing.dakai(this.dataSource[i],"1")
              }else if( this.dataSource[i].jclx == "对公贷款" ){
                console.log("对公贷款")
                this.$refs.duigong.dakai(this.dataSource[i],"1")
              }
          }
        }
      }
