<template>
  ...
  >
    ...
    <!-- <div
      ...
      v-show="provinceData.length>0"
      v-if="nextShenHeGangWei"
    >
    ...
    <div class="jieguo" style="margin-top:18px;" v-if="fenfaHide">
    ...
    dakai(a, b, c, d) {
      ...
      getAction('/process/getConfiguration', obj).then(res => {
        ...
        if (this.xuanzeName == '退回至') {
          ...
          this.xian = res.selectApprovalUser
        }

        this.nextShenHeRenYuan = res.selectApprovalUser
        this.tuihuizhia = res.canReturnTo
        this.currentTaskName = res.currentTaskName
        this.variableName = res.variableName
        if (res.currentTaskName == '分发岗') {
          this.fenfaHide = false
        } else {
          this.fenfaHide = true
        }
        ...
        if (res.users.length > 0 && res.selectApprovalUser == true) {
